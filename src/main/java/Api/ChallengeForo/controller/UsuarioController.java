...
import Api.ChallengeForo.domain.usuario.*;
import Api.ChallengeForo.domain.usuario.actualizar.ValidarActualizarUsuario;
import Api.ChallengeForo.domain.usuario.crear.ValidarCrearUsuario;
...
public class UsuarioController {
    ...
    public ResponseEntity<UsuarioDto> crearUsuario(@RequestBody @Valid CrearUsuarioDto crearUsuarioDTO, UriComponentsBuilder uriBuilder){
    ...
    public ResponseEntity<Page<UsuarioDto>> leerTodosUsuarios(@PageableDefault(size = 5, sort = {"id"}) Pageable pageable){
        var pagina = repository.findAll(pageable).map(UsuarioDto::new);
        return ResponseEntity.ok(pagina);
    }

    @GetMapping
    @Operation(summary = "Lista solo usuarios habilitados")
    public ResponseEntity<Page<UsuarioDto>> leerUsuariosActivos(@PageableDefault(size = 5, sort = {"id"}) Pageable pageable){
        var pagina = repository.findAllByEnabledTrue(pageable).map(UsuarioDto::new);
        return ResponseEntity.ok(pagina);
    }

    @GetMapping("/username/{username}")
    @Operation(summary = "Lee un único usuario por su nombre de usuario")
    public ResponseEntity<UsuarioDto> leerUnUsuario(@PathVariable String username){
    ...
    public ResponseEntity<UsuarioDto>leerUnUsuario(@PathVariable Long id){
    ...
    public ResponseEntity<UsuarioDto> actualizarUsuario(@RequestBody @Valid ActualizarUsuarioDto actualizarUsuarioDTO, @PathVariable String username){
